...
class M_Metod extends C_Controller
{
    ...
    public function action_add_reviews(){
        ...
        $this->pdo->insert('reviews',['name'=>$name,'text'=>$text]);
    }
    ...
    public function action_bay_basket(){
        if($_SESSION['basket']){
            foreach($_SESSION['basket'] as $basket){
                ...
                $goods_id = $basket['id'];
                $count = $basket['count'];
                $object = ['user'=>$user,'goods_id'=>$goods_id,'count'=>$count,];
                $this->pdo->insert('purchases',$object);
            }
        }
        $_SESSION['basket'] = [];?>
    ...
    public function action_admin_user_edit(){
        ...
        if($user["admin"]==="true"){
            $this->pdo->update("users",["admin"=>'false'],['id_user'=>$id]);
        }elseif($user["admin"]==="false"){
            $this->pdo->update("users",["admin"=>'true'],['id_user'=>$id]);
        };
    }
    ...
    public function action_admin_goods_save(){
        $set = ['title' => $_POST['title'], 'text' => $_POST['text']];
        $where = ['goods.id' => $_GET['id']];
        $this->pdo->update("users",$set,$where);
    }

    public function action_admin_goods_add(){
        $this->metod = "content";
        $this->pdo->insert('goods',['id'=>'NULL']);
    ...
    public function action_admin_goods_remove(){
        ...
        $id = $_GET['id'];
        $pdo = $this->pdo;
        $pdo->delete("goods",['id'=>$id]);
        // $pdo->query("DELETE FROM `goods` WHERE `id` = $id")->fetch();
